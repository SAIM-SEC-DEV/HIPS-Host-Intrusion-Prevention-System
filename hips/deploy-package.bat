...
echo powershell -Command "(Get-Content 'hips-agent.properties') -replace 'server.url=.*', 'server.url=http://%%SERVER_IP%%/hips' | Set-Content 'hips-agent.properties'" >> dist\setup-agent.bat
(
echo.
echo echo.
