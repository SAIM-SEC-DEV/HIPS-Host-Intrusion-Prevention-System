...
echo  [4/7] Deploying Server Files and Starting Apache...

REM Check if HIPS server folder exists in htdocs
if not exist "%XAMPP_DIR%\htdocs\hips" (
    echo  [INFO] Deploying HIPS server files to XAMPP htdocs...
    mkdir "%XAMPP_DIR%\htdocs\hips" >nul 2>&1
    xcopy /E /Y /I "!HIPS_DIR!\server\*" "%XAMPP_DIR%\htdocs\hips\" >nul 2>&1
    if !errorlevel!==0 (
        echo  [OK] Server files deployed successfully.
        echo  [INFO] Verifying local access: http://localhost/hips/api/ping.php
    ) else (
        echo  [WARN] Failed to deploy server files automatically.
        echo  Please manually copy the 'server' folder to '%XAMPP_DIR%\htdocs\hips'.
    )
) else (
    echo  [OK] HIPS server files already present in htdocs.
    REM Proactively update files to ensure latest version
    xcopy /E /Y /I "!HIPS_DIR!\server\*" "%XAMPP_DIR%\htdocs\hips\" >nul 2>&1
    echo  [INFO] Server files synchronized.
)
