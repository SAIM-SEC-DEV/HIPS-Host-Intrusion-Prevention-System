        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            ...
            try (DatagramSocket socket = new DatagramSocket()) {
                ...
                if (response.startsWith("HIPS_SERVER|")) {
                    // We ignore the IP embedded in the string since the server might have multiple adapters (like VirtualBox).
                    // Instead, we use the actual source IP address of the UDP packet we received!
                    String actualServerIp = receivePacket.getAddress().getHostAddress();
                    String serverUrl = "http://" + actualServerIp + "/hips";
                    
                    System.out.println("[HIPS-DISCOVERY] ✓ Server found at: " + serverUrl);
                    return serverUrl;
                }
