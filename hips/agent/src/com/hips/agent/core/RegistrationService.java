...
public class RegistrationService {
    ...
    public boolean register() {
        ...
        payload.put("host_os",       detectHostOs());
    ...
    private boolean reRegister() {
        ...
        if (response != null && response.has("status")
            ...
            // Sync credentials from server to ensure local config is correct
            if (response.has("agent_id"))   config.setAgentId(response.get("agent_id").getAsInt());
            if (response.has("agent_uuid")) config.setAgentUuid(response.get("agent_uuid").getAsString());
            if (response.has("auth_token")) config.setAuthToken(response.get("auth_token").getAsString());
            
            config.saveConfig(); // Persist the corrected/synced credentials
    ...
    /**
     * Detects the host operating system type for cross-platform identification.
