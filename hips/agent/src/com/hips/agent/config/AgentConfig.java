...
public class AgentConfig {
    ...
    // YARA configuration
    private String yaraBinaryPath;
    private String yaraRulesPath;

    // osquery integration configuration
    private boolean osqueryEnabled;
    private String osqueryBinaryPath;
    private int osqueryTimeoutSec;
    ...
    private void loadConfig() {
        ...
        // Parse YARA configuration
        this.yaraBinaryPath = props.getProperty("yara.binary.path", "C:\\HIPS\\bin\\yara64.exe");
        this.yaraRulesPath  = props.getProperty("yara.rules.path", "C:\\Users\\Jawad KHAN\\Desktop\\HIPS-OOP\\rules");

        // osquery configuration (defaults to disabled with auto-detected path)
        this.osqueryEnabled   = Boolean.parseBoolean(props.getProperty("osquery.enabled", "false"));
        String defaultOsqPath = detectDefaultOsqueryPath();
        this.osqueryBinaryPath = props.getProperty("osquery.binary.path", defaultOsqPath);
        try {
            this.osqueryTimeoutSec = Integer.parseInt(props.getProperty("osquery.timeout.sec", "15"));
        } catch (NumberFormatException e) {
            this.osqueryTimeoutSec = 15;
        }
    }

    /**
     * Auto-detects the default osqueryi binary path based on the OS.
     */
    private static String detectDefaultOsqueryPath() {
    ...
    public void saveConfig() {
        ...
        props.setProperty("auth.token",          authToken != null ? authToken : "");
        ...
        props.setProperty("yara.binary.path",     yaraBinaryPath != null ? yaraBinaryPath : "");
        props.setProperty("yara.rules.path",      yaraRulesPath != null ? yaraRulesPath : "");
        props.setProperty("osquery.enabled",       String.valueOf(osqueryEnabled));
        props.setProperty("osquery.binary.path",   osqueryBinaryPath != null ? osqueryBinaryPath : "");
        props.setProperty("osquery.timeout.sec",   String.valueOf(osqueryTimeoutSec));
    ...
    public String   getYaraBinaryPath()       { return yaraBinaryPath; }
    public String   getYaraRulesPath()        { return yaraRulesPath; }
    ...
    public void setYaraBinaryPath(String path)          { this.yaraBinaryPath = path; }
    public void setYaraRulesPath(String path)           { this.yaraRulesPath = path; }

    // osquery getters and setters
    public boolean isOsqueryEnabled()                   { return osqueryEnabled; }
    public String  getOsqueryBinaryPath()               { return osqueryBinaryPath; }
    public int     getOsqueryTimeoutSec()               { return osqueryTimeoutSec; }
    public void setOsqueryEnabled(boolean enabled)      { this.osqueryEnabled = enabled; }
    public void setOsqueryBinaryPath(String path)       { this.osqueryBinaryPath = path; }
    public void setOsqueryTimeoutSec(int sec)           { this.osqueryTimeoutSec = sec; }
