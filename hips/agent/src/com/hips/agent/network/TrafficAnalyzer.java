...
public class TrafficAnalyzer {
    ...
    public void detectTrafficSpike() {
        ...
        if (average > 0 && current > average * TRAFFIC_SPIKE_MULTIPLIER) {
            ...
            metadata.put("current_established", current);
            metadata.put("average_established", Math.round(average * 10.0) / 10.0);
            metadata.put("spike_ratio", Math.round((current / average) * 10.0) / 10.0);
    ...
    public void detectDataExfiltration(List<Map<String, String>> connections) {
        ...
        for (Map.Entry<String, Integer> entry : outboundCounts.entrySet()) {
            if (entry.getValue() >= EXFIL_OUTBOUND_THRESHOLD) {
                ...
                Map<String, Object> metadata = new HashMap<>();
                metadata.put("destination_ip", entry.getKey());
                metadata.put("connection_count", entry.getValue());
                metadata.put("threshold", EXFIL_OUTBOUND_THRESHOLD);
