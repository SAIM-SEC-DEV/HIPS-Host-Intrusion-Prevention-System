...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="memory-monitor.php" class="nav-item <?= $currentPage === 'memory-monitor' ? 'active' : '' ?>">
            <span class="nav-icon"><i class="fa-solid fa-memory"></i></span>
            <span>Memory Analysis</span>
        </a>

        <a href="hardware-assets.php" class="nav-item <?= $currentPage === 'hardware-assets' ? 'active' : '' ?>">
            <span class="nav-icon"><i class="fa-solid fa-server"></i></span>
            <span>Hardware Assets</span>
        </a>

        <a href="virus-total.php" class="nav-item <?= $currentPage === 'virus-total' ? 'active' : '' ?>">
            <span class="nav-icon"><i class="fa-solid fa-shield-halved"></i></span>
            <span>Intel Analyzer</span>
        </a>

        <a href="vt-safe-check.php" class="nav-item <?= $currentPage === 'vt-safe-check' ? 'active' : '' ?>">
            <span class="nav-icon"><i class="fa-solid fa-microscope"></i></span>
            <span>Safe VT Check</span>
        </a>

        <a href="usb-monitor.php" class="nav-item <?= $currentPage === 'usb-monitor' ? 'active' : '' ?>">
            <span class="nav-icon"><i class="fa-solid fa-usb"></i></span>
            <span>USB Monitor</span>
        </a>

        <a href="rootkit-scanner.php" class="nav-item <?= $currentPage === 'rootkit-scanner' ? 'active' : '' ?>">
            <span class="nav-icon"><i class="fa-solid fa-user-secret"></i></span>
        ...
        <a href="mitre-heatmap.php" class="nav-item <?= $currentPage === 'mitre-heatmap' ? 'active' : '' ?>">
            <span class="nav-icon"><i class="fa-solid fa-chess-board"></i></span>
            <span>MITRE Heatmap</span>
        </a>
    ...
    <div class="sidebar-footer">
        ...
        <button class="theme-toggle-btn" id="themeToggle" title="Switch theme">
            <span class="theme-icon" id="themeIcon"><i class="fa-solid fa-moon"></i></span>
            <span id="themeLabel">Dark Mode</span>
        </button>
        <div style="display:flex; align-items:center; justify-content:space-between; margin-top:12px;">
            <span><i class="fa-solid fa-user-shield"></i> <?= htmlspecialchars($adminName) ?></span>
            <a href="logout.php" style="color:var(--text-muted); font-size:12px;"><i class="fa-solid fa-right-from-bracket"></i> Logout</a>
        </div>
    </div>
</aside>
...
(function() {
    ...
    // If epic dragon intro is active (first-time dashboard), hide preloader IMMEDIATELY
    if (document.getElementById('hipsIntro') && !localStorage.getItem('hips_intro_played')) {
        preloader.style.display = 'none';
        return;
    }

    // Fast dismiss for sensible loader on window.load
