    ...
    if ($tab === 'account') {
        ...
        if (!empty($newPassword) && $newPassword === $confirmPassword) {
            ...
            if ($user && password_verify($currentPassword, $user['password_hash'])) {
                ...
                AuditLogger::log("Password Change", "User", $adminUsername);
        ...
        AuditLogger::log("Settings Update", "Category", $tab);
        $settingsSaved = true;
    }
}
...
<div class="app-layout">
    ...
    <main class="main-content">
        ...
        <div style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);border-radius:var(--radius-sm);padding:12px;margin-bottom:16px;color:var(--severity-low);font-size:13px;"><i class="fa-solid fa-check"></i> Settings saved successfully.</div>
        <?php endif; ?>

        <?php if (isset($passwordSuccess)): ?>
        <div style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);border-radius:var(--radius-sm);padding:12px;margin-bottom:16px;color:var(--severity-low);font-size:13px;"><i class="fa-solid fa-check"></i> <?= $passwordSuccess ?></div>
        <?php endif; ?>
        ...
        <div class="tab-nav animate-fadeIn delay-1">
            <a href="?tab=general" class="tab-btn <?= $activeTab==='general'?'active':'' ?>"><i class="fa-solid fa-sliders"></i> General</a>
            <a href="?tab=security" class="tab-btn <?= $activeTab==='security'?'active':'' ?>"><i class="fa-solid fa-shield-halved"></i> Security Rules</a>
            <a href="?tab=notifications" class="tab-btn <?= $activeTab==='notifications'?'active':'' ?>"><i class="fa-solid fa-bell"></i> Notifications</a>
            <a href="?tab=api" class="tab-btn <?= $activeTab==='api'?'active':'' ?>"><i class="fa-solid fa-key"></i> API & Agents</a>
            <a href="?tab=account" class="tab-btn <?= $activeTab==='account'?'active':'' ?>"><i class="fa-solid fa-user-shield"></i> Account</a>
        </div>

        <!-- General Settings -->
        <?php if ($activeTab === 'general'): ?>
        <div class="card animate-fadeIn delay-2">
            <div class="card-title"><span class="icon"><i class="fa-solid fa-sliders"></i></span> General Configuration</div>
            <form method="POST">
        ...
        <!-- Safety Monitor: Show Current IP -->
        <div class="card animate-fadeIn delay-1" style="background:linear-gradient(135deg, rgba(139,92,246,0.1), rgba(16,185,129,0.05)); border:1px solid rgba(139,92,246,0.2);">
            <div style="display:flex; align-items:center; gap:15px;">
                <div style="width:40px; height:40px; background:var(--accent-violet); border-radius:10px; display:flex; align-items:center; justify-content:center; color:#fff; font-size:18px;">
                    <i class="fa-solid fa-network-wired"></i>
                </div>
                <div>
                    <div style="font-size:12px; color:var(--text-muted); text-transform:uppercase; letter-spacing:1px; margin-bottom:2px;">Your Current IP Address</div>
                    <div style="font-size:18px; color:var(--text-primary); font-weight:700; font-family:'JetBrains Mono',monospace;">
                        <?php 
                        $curIp = $_SERVER['REMOTE_ADDR'] ?? 'Unknown';
                        if ($curIp === '::1') $curIp = '127.0.0.1';
                        echo $curIp;
                        ?>
                    </div>
                </div>
                <div style="margin-left:auto; text-align:right;">
                    <span class="badge low" style="padding:6px 12px; border-radius:20px;"><i class="fa-solid fa-circle-check"></i> Connection Secure</span>
                    <div style="font-size:10px; color:var(--text-muted); margin-top:5px;">Ensure this IP is whitelisted before enabling restrictions.</div>
                </div>
            </div>
        </div>

        <div class="card animate-fadeIn delay-2">
            <div class="card-title"><span class="icon"><i class="fa-solid fa-shield-halved"></i></span> Security Rule Configuration</div>
            <form method="POST">
                <?php CsrfHandler::insertField(); ?>
                <input type="hidden" name="tab" value="security">
                <?php if (isset($settings['security'])): foreach ($settings['security'] as $key => $s):
                    $isToggle = in_array($key, ['file_monitor_enabled', 'network_monitor_enabled', 'ip_whitelist_enabled']);
                ?>
        ...
        <div class="card animate-fadeIn delay-2">
            <div class="card-title"><span class="icon"><i class="fa-solid fa-bell"></i></span> Notification Settings</div>
            <form method="POST">
                <?php CsrfHandler::insertField(); ?>
        ...
        <div class="card animate-fadeIn delay-2">
            <div class="card-title"><span class="icon"><i class="fa-solid fa-key"></i></span> API Token Management</div>
        ...
        <div class="card animate-fadeIn delay-2">
            <div class="card-title"><span class="icon"><i class="fa-solid fa-user-shield"></i></span> Account & Security</div>
            <form method="POST">
                <?php CsrfHandler::insertField(); ?>
