...
$agents = $pdo->query(
    "SELECT a.*,
     ...
     FROM agents a ORDER BY a.status DESC, a.hostname"
)->fetchAll();
...
<div class="app-layout">
    ...
    <main class="main-content">
        ...
        <div class="stat-grid">
            <div class="stat-card cyan animate-fadeIn delay-1"><div class="stat-icon">🖥</div><div class="stat-value"><?= $totalAgents ?></div><div class="stat-label">Total Registered</div></div>
            <div class="stat-card green animate-fadeIn delay-2"><div class="stat-icon">✓</div><div class="stat-value"><?= $onlineCount ?></div><div class="stat-label">Online</div></div>
            <div class="stat-card amber animate-fadeIn delay-3"><div class="stat-icon">⏸</div><div class="stat-value"><?= $offlineCount ?></div><div class="stat-label">Offline</div></div>
            <div class="stat-card red animate-fadeIn delay-4"><div class="stat-icon">🔔</div><div class="stat-value"><?= $alertsToday ?></div><div class="stat-label">Alerts Today</div></div>
        </div>
        ...
        <div class="grid-2">
            ...
            <div class="card animate-fadeIn delay-<?= min($i + 1, 5) ?>" style="border-left:4px solid <?= $ag['status']==='online' ? 'var(--accent-blue)' : 'var(--status-offline)' ?>;">
                ...
                <div style="display:flex;gap:8px;">
                    <a href="events.php?agent=<?= $ag['id'] ?>" class="btn btn-secondary btn-sm">📋 View Logs</a>
                    <a href="commands.php?agent=<?= $ag['id'] ?>" class="btn btn-secondary btn-sm">⌨ Send Command</a>
                </div>
            </div>
